...
import { LuSearch as Search, LuShoppingCart as ShoppingCart, LuUser as User, LuShieldCheck as ShieldCheck, LuHelpCircle as HelpCircle, LuStore as Store, LuX as X, LuInfo as Info } from 'react-icons/lu';
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 font-sans shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            <span className="text-xl md:text-2xl font-black font-sans tracking-tight text-slate-900 flex items-center">
              TENBUK<span className="text-orange-500 text-base md:text-lg ml-0.5 font-bold">.com</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <button
              ...
              className="flex items-center justify-center font-bold px-2 py-1.5 md:px-3 rounded-md border border-slate-200 text-xs text-slate-700 bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer font-mono"
            ...
            <button
              id="btn-cart-trigger"
              onClick={openCart}
              className="p-1.5 md:p-2 text-slate-700 hover:text-orange-600 hover:bg-slate-50 rounded-full transition-all relative cursor-pointer"
              title="Open Order Summary"
            >
              <ShoppingCart className="h-5 w-5 md:h-6 md:w-6 stroke-[1.8]" />
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-orange-600 text-white font-black text-[9px] md:text-[10px] w-4 h-4 md:w-5 md:h-5 flex items-center justify-center rounded-full border-2 border-white animate-pulse">
                  {cartCount}
                </span>
              )}
            </button>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 p-1 md:p-1.5 rounded-full hover:bg-slate-50 transition-all border border-slate-100 cursor-pointer text-slate-800 font-sans"
                title="Change Workspace Role"
              >
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-xs md:text-sm">
        ...
        <div className="mt-3 md:hidden relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-full bg-slate-50 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm text-slate-800 transition-all font-sans"
          />
        </div>
      </div>
    </header>
